    ...
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            while (true)
            {
                ...
                else if (UserChoiseCategory == 3) products.AddRange(Db.Configurations);
                
                while (true)
                {
                    var filterChoise = choice.UserChoiceFilter();
                    if (filterChoise == 1)
                    {
                        while (true)
                        {
                            choice.ShowProducts(products);
                            var userContinue = choice.UserContinueShopingAction();
                            if (userContinue == 1) continue;
                            else if (userContinue == 2) break;
                    ...
                    else if (filterChoise == 3)
                    {
                        while (true)
                        {
                            if (UserChoiseCategory == 1) choice.FilterByType(Db.Parts);
                            else if(UserChoiseCategory == 2) choice.filterByTypeModules(Db.Modules);
                            else if(UserChoiseCategory == 3) choice.filterByTypeConfigurations(Db.Configurations);
